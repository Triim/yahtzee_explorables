    ...
    return (
      <div className="lin-model">
        <p className="lin-note">
          {tr('среднее очков со строки за партию', 'average points a row brings per game')}
        </p>
  ...
  beats: [
    {
      ...
      payoff:
        'Это та самая линейность ожидания, что усредняла число шестёрок, — только теперь для всех тринадцати строк сразу. Она верна всегда, даже когда строки связаны:\n[[$E[\\sum X_i] = \\sum E[X_i]$]]\nСами столбики — это среднее, что строка приносит за партию при разумной игре. Верхние читаются легко: метишь примерно по три кости на грань (та самая логика порога 63), оттого шестёрки тянут под двадцать, а единицы скромные — около двух. Нижние стоят свой номинал, помноженный на то, как часто рука и вправду доходит. Отсюда напрашивается рецепт: набей каждую строку на максимум — и сумма выйдет максимальной.',
      gate: { kind: 'step' },
    },
    {
      id: 'B6.1W',
